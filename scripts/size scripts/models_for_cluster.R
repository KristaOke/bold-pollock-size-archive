...
saveRDS(cor_all1ML, file=paste(wd,"/scripts/size scripts/model_output_all-ages_random-cohort_ML.rds", sep=""))
cor_all1ML_cAIC <- cAIC(cor_all1ML)
saveRDS(cor_all1ML_cAIC, file=paste(wd,"/scripts/size scripts/model_output_cluster/cAIC_all-ages_random-cohort_ML.rds", sep=""))

#full model all ages w global
G_all1ML <- gamm4(log_sc_weight ~  s(sst.amj, k=4) + s(sst.amj, by=AGE, k=4) + t2(LONGITUDE, LATITUDE) + s(julian, k = 4),
                random=~(1|YEAR/HAUL) + (1|cohort), data=lagdat, REML=FALSE) 
saveRDS(G_all1ML, file=paste(wd,"/scripts/size scripts/model_output_all-ages_random-cohort_wglobal_ML.rds", sep=""))
G_all1ML_cAIC <- cAIC(G_all1ML)
saveRDS(G_all1ML_cAIC, file=paste(wd,"/scripts/size scripts/model_output_cluster/cAIC_all-ages_random-cohort_wglobal_ML.rds", sep=""))

#global only model
GonlyML <- gamm4(log_sc_weight ~  s(sst.amj, k=4) +  t2(LONGITUDE, LATITUDE) + s(julian, k = 4),
                  random=~(1|YEAR/HAUL) + (1|cohort), data=lagdat, REML=FALSE) 
saveRDS(GonlyML, file=paste(wd,"/scripts/size scripts/model_output_all-ages_random-cohort_onlyglobal_ML.rds", sep=""))
GonlyML_cAIC <- cAIC(GonlyML)
saveRDS(GonlyML_cAIC, file=paste(wd,"/scripts/size scripts/model_output_cluster/cAIC_all-ages_random-cohort_onlyglobal_ML.rds", sep=""))
...
saveRDS(cor_11, file=paste(wd,"/scripts/size scripts/model_output_age11under_random-cohort.rds", sep=""))
...
saveRDS(cor_9, file=paste(wd,"/scripts/size scripts/model_output_age9under_random-cohort.rds", sep=""))
